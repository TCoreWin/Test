...
public class bicycle_code : MonoBehaviour
{
    ...
    */
    ...
    void Start()
    {
        ...
        rigidbody.centerOfMass = new Vector3(CoM.localPosition.x, CoM.localPosition.y, CoM.localPosition.z);// now Center of Mass(CoM) is alligned to GameObject "CoM"
        rigidbody.inertiaTensor = setInitialTensor;////this string is necessary for Unity 5.3f with new PhysX feature when Tensor decoupled from center of mass
    ...
    void FixedUpdate()
    {
        ...
        if (!crashed)
        {
            rigidbody.drag = rigidbody.velocity.magnitude / 210 * airRes; // when 250 bike can easy beat 200km/h // ~55 m/s
            rigidbody.angularDrag = 14 + rigidbody.velocity.magnitude / 20;
        }

        //determinate the bike speed in km/h
        bikeSpeed = Mathf.Round((rigidbody.velocity.magnitude * 3.6f) * 10) * 0.1f; //from m/s to km/h
        ...
        if (!crashed && outsideControls.Vertical > 0.9f && !isReverseOn)// acceleration >0.9f throttle for wheelie	
        {
            ...
            rigidbody.angularDrag = 20;//for wheelie stability
        ...
        */
        ...
        if (!crashed && outsideControls.Vertical < 0 && !isFrontWheelInAir)
        {
            ...
            if (CoM.localPosition.z >= 0.2f + (rigidbody.mass / 500))
            {
                CoM.localPosition = new Vector3(CoM.localPosition.x, 0.2f + (rigidbody.mass / 500), CoM.localPosition.z);
            }
        ...
        {
            rigidbody.drag = 0.1f; // when 250 bike can easy beat 200km/h // ~55 m/s
